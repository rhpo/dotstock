...
void AddProductUI(MyForm^ form, String^ ID, String^ name, int Quantity) {
	...
	if (Produits->ColumnCount < 3) {
		...
		Produits->Columns[0]->HeaderText = "Quantité en Stock";
	}
...
void AddVenteUI(MyForm^ form, Vente^ vente) {
	...
	if (Ventes->ColumnCount < 9) {
		...
		Ventes->Columns[3]->HeaderText = "Quantité";
...
void MyForm::AddProductEvent(System::Object^  sender, System::EventArgs^  e) {
	...
	if (productName->Length < 3 || productName->Length > 100) 
		...
		MessageBox::Show("Veuiller entrer une quantité de produit valide (entre 1 et 50,000).", "Erreur", MessageBoxButtons::OK, MessageBoxIcon::Error);
	else {
		...
		MessageBox::Show("Le produit '" + productName + "' a été ajouté avec succèss.", "Réussite", MessageBoxButtons::OK, MessageBoxIcon::Information);
	}
...
void MyForm::AddVente(System::Object^ sender, System::EventArgs^  e) {
	...
	if (String::IsNullOrEmpty(ID))
		...
		if (quantite > Found->Quantity)
			MessageBox::Show("Quantité de '" + Found->Name + "' Insuffisante.", "Insuffisant", MessageBoxButtons::OK, MessageBoxIcon::Error);
		else {
...
void MyForm::button5_Click(System::Object^  sender, System::EventArgs^  e) {
	...
	if (String::IsNullOrEmpty(ID))
		...
		if (Found == nullptr)
			...
			QuantiteVente->Value = QuantiteVente->Minimum;

			if (quantite >= Found->Quantity) {
				System::Windows::Forms::DialogResult sup = MessageBox::Show("Etes-vous sûre de supprimer '" + Found->Name + "' ?", "Supprimer: " + Found->Name, MessageBoxButtons::YesNo, MessageBoxIcon::Warning);
			
				if (sup == System::Windows::Forms::DialogResult::Yes) {
					Found->Quantity = 0;
					VenteProduit->Text = "";

					Dataset::products->Remove(Found);
				}
			}

			else {

				System::Windows::Forms::DialogResult dim = MessageBox::Show("Etes-vous sûre de vider " + quantite + " éléments de '" + Found->Name + "' ?", "Vider: " + Found->Name, MessageBoxButtons::YesNo, MessageBoxIcon::Question);
			
				if (dim == System::Windows::Forms::DialogResult::Yes) {
					Found->Quantity -= quantite;
					VenteProduit->Text = "";
					VenteProduit->Focus();
				}
...
void MyForm::ChangeQuantity(System::Object^  sender, System::EventArgs^  e) {
	...
	if (String::IsNullOrEmpty(id)) {
		...
		MessageBox::Show("Vente avec (ID=" + id + ") n'est pas trouvé.", "Oops! Vérifiez l'ID...", MessageBoxButtons::OK, MessageBoxIcon::Error);
	}
		...
		if (v->_Produit->Quantity + addToProduct < 0)
			MessageBox::Show("La quantité de '" + v->_Produit->Name + "' ne suffit pas pour atteindre x" + quantity + " éléments.", "Quantité Insuffisante...", MessageBoxButtons::OK, MessageBoxIcon::Error);
		
		else {

			// if delete vente
			if (!quantity) {
				System::Windows::Forms::DialogResult sVente = MessageBox::Show("Etes-vous sûre de supprimer la Vente ?", "Supprimer la Vente: " + v->ID, MessageBoxButtons::YesNo, MessageBoxIcon::Warning);
			
				if (sVente == System::Windows::Forms::DialogResult::Yes) {
					v->_Produit->Quantity += addToProduct;
					v->Quantity -= addToProduct;
					v->Recalculate(TAUX_TVA);

					Dataset::ventes->Remove(v);
